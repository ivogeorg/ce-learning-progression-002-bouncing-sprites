...
3. `[<lernact-prac>]`**[Optional challenge, max 3 extra step points]** Make your `BrightPoint` blink at one of 3 different rates. Create a few sprites and have them move about, blinking each at its rate. _Hint: You will need to use `input.runningTime` to decide whether to plot or unplot depending on the blink period. Look at the implementation of blinking for the `game.LedSprite` class in [`game.ts`](https://github.com/microsoft/pxt-microbit/blob/master/libs/core/game.ts).    
